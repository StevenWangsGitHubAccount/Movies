...
export default class Recommendations extends React.Component {
    		...
    		fetch("http://localhost:8081/recommendations/" + this.state.movieName,
    {
      ...
      let recDivs = movieResults.map((RecommendationsRow, i) =>
      <div key={i} className="movieResults">
        <div className="title">{RecommendationsRow.title}</div>
        <div className="id">{RecommendationsRow.id}</div>
        <div className="rating">{RecommendationsRow.rating}</div>
        <div className="votes">{RecommendationsRow.vote_count}</div>
      </div>);
